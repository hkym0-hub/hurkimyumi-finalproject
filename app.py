...
위 메뉴 목록 중에서 딱 1개만 골라서 아래 형식으로 답해주세요:
추천메뉴: [메뉴이름]
이유: [2-3문장으로 선택 이유]
            ...
            with st.spinner("AI가 고민 중...🤔"):
                try:
                    ...
                    if matched:
                        ...
                        st.warning(f"AI 응답: {text[:200]}")
                        picked = random.choice(menus)
                        add_history(picked, "🤖 AI 추천(랜덤 fallback)")
                        result_card(picked, "🤖 AI 추천")
                except Exception as e:
                    st.error(f"AI 호출 실패: {e}. 랜덤으로 추천할게요!")
                    picked = random.choice(menus)
                    add_history(picked, "🤖 AI 추천(오류)")
                    result_card(picked, "🤖 AI 추천")
